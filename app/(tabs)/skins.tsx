import { StyleSheet, Image, Platform, View, TouchableHighlight, FlatList, TextInput, Modal, TouchableOpacity } from 'react-native';
...
export default function Skins() {
  ...
  const handleSearch = (query: string) => {
    ...
    if (searchTerms.length === 0) {
      ...
      const filteredSkins = skins.filter((skin: any) =>
        searchTerms.every(term => skin.displayName.toLowerCase().includes(term))
      );
  ...
  return (
    <View style={{ backgroundColor: Colors.dark.background, flexGrow: 1, justifyContent: 'center', alignItems: 'center', gap:15 }}>
      <View style={{flexDirection: 'row', width: '100%', justifyContent: 'center', marginTop: 30,}}>
          ...
          />
          <TouchableOpacity onPress={handleSearchIcon} style={{padding: 5, position: 'absolute', right: '10%', marginTop: 8}}>
            <MaterialIcons style={{fontSize: 30, color: '#888'}}>{searchQuery ? 'close' : 'search'}</MaterialIcons>
          </TouchableOpacity>
      </View>

      <Text style={{fontSize: 16, fontFamily: 'Rubik600', color: Colors.text.highlighted, textTransform: 'uppercase'}}>( {visibleSkins.length} Skins Founded )</Text>
      ...
      <FlatList
          ...
          <View style={{ alignItems: 'center', marginBottom: 15 }}>
            ...
            >
              <View style={{
                ...
                <Text style={{
                  width: '60%', fontFamily: 'Rubik500', color: 'white', fontSize: 20, flexWrap: 'wrap'
                }}>
                  {item.displayName}
                </Text>
        ...
        ListEmptyComponent={() => (
          <Text style={{ color: 'white', fontSize: 18, fontFamily: 'Rubik500', textAlign: 'center' }}>No skins found.</Text>
        )}
      />
